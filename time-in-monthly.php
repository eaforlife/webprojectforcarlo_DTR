...
<html>
<head>
<title>Date Time Record - Time In - Monthly</title>
...
<?php
	...
	if(isset($_POST['date']) && isset($_POST['days']) && isset($_POST['year']) && isset($_POST['month']) && isset($_POST['empListId'])) {
		$empID = cleanTxt($_POST['empListId']);
		$selDays = cleanTxt($_POST['days']); // 7
		$selDate = $_POST['year'] . "-" . $_POST['month'] . "-" . $_POST['date'] . " 00:00:01"; // YearInFull-MonthInNumber-DaysWithoutLeadZero Hours:Minute:Seconds
		$testDate = new DateTime($selDate);
		$ctrz = 0;
		if(!empty($selDate) && !empty($selDays)) {
			for($x = 0; $x <= $selDays - 1; $x++) {
				...
				if($queryDT->num_rows > 0) {
					...
					if($myConn->query($addDt) === TRUE) {
						$outMsg = "<div id='results-success'><br><hr><br><p><span class='txt-success' id='results'>Successfully addded days for employee ID: <strong>$empID</strong>!</span></p></div>";
					} else {
						$ctr += 1;
					}
			...
			$outMsg = "<div id='results-err'><br><hr><br><p class='txt-danger'>Something went wrong while adding your time(s). Try again later.</p></div>";
		}
	}
...
<div class="container">
	<div class="row">
		...
		<form method="post" id="frmIn">
			...
			<select id="fromYear" class="form-flex" name="year" required>
				<option value='2020'>2020</option>
				<option value='2019'>2019</option>
				<option value='2018'>2018</option>
			</select><br>
			<label for="fromMonth">Month: </label>
			<select id="fromMonth" name="month" class="form-flex" required>
			<?php
				// Auto populate month
				$months = array();
			...
			<label for="fromDate">From(1-<span id="maxday"><?php $day=date('Y-m-d'); echo date('t',strtotime($day)); ?></span>): </label>&nbsp;<input type="number" name="date" id="fromDate" min="1" max="<?php $day=date('Y-m-d'); echo date('t',strtotime($day)); ?>" value="1" step="1" required><br>
			<label for="dateDial">Days(7-45): </label>&nbsp;<input type="number" name="days" id="dateDial" min="7" max="45" step="1" value="14" required><br><br>
			<input type="submit" class="btn btn-lg btn-success" id="checkIn" value="Time In - Monthly"></button>
...
document.getElementById("fromMonth").onchange = function() {
	var month = document.getElementById("fromMonth").value;
	var year = document.getElementById("fromYear").value;
	var days = new Date(year, month, 0).getDate();
	
	document.getElementById("maxday").innerHTML = days;
	document.getElementById("fromDate").setAttribute("max", days);
};
